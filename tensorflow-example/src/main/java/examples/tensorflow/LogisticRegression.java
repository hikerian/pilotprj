...
import examples.tensorflow.model.datasets.mnist.MnistDataset;


/**
 * @see https://github.com/tensorflow/java-models/blob/master/src/main/java/org/tensorflow/model/examples/dense/SimpleMnist.java
 */
public class LogisticRegression implements Runnable {
	...
	public static void main(String[] args) {
		...
		try (Graph graph = new Graph()) {
			LogisticRegression mnist = new LogisticRegression(graph, dataset);
			mnist.run();
		}
	}
	...
	private static TFloat32 preprocessImages(ByteNdArray rawImages) {
		...
		return tf.math.div(tf.reshape(tf.dtypes.cast(tf.constant(rawImages), TFloat32.class), tf.array(-1L, imageSize)),
				tf.constant(255.0f)).asTensor();
	}

	private static TFloat32 preprocessLabels(ByteNdArray rawLabels) {
		Ops tf = Ops.create();

		// Map labels to one hot vectors where only the expected predictions as a value
		// of 1.0
		return tf.oneHot(tf.constant(rawLabels), tf.constant(MnistDataset.NUM_CLASSES), tf.constant(1.0f),
				tf.constant(0.0f)).asTensor();
	}

	private final Graph graph;
	private final MnistDataset dataset;

	private LogisticRegression(Graph graph, MnistDataset dataset) {
		this.graph = graph;
	...
	public void run() {
		...
		// Create placeholders and variables, which should fit batches of an unknown
		// number of images
		...
		Mean<TFloat32> crossEntropy = tf.math
				.mean(tf.math.neg(tf.reduceSum(tf.math.mul(labels, tf.math.log(softmax)), tf.array(1))), tf.array(0));
		...
		Operand<TFloat32> accuracy = tf.math.mean(tf.dtypes.cast(tf.math.equal(predicted, expected), TFloat32.class),
				tf.array(0));
		...
		try (Session session = new Session(graph)) {
			...
			for (ImageBatch trainingBatch : dataset.trainingBatches(TRAINING_BATCH_SIZE)) {
				try (TFloat32 batchImages = preprocessImages(trainingBatch.images());
					...
					session.runner().addTarget(minimize).feed(images.asOutput(), batchImages)
							.feed(labels.asOutput(), batchLabels).run();
				}
			}
			...
			try (TFloat32 testImages = preprocessImages(testBatch.images());
					...
					var result = session.runner().fetch(accuracy).feed(images.asOutput(), testImages)
							.feed(labels.asOutput(), testLabels).run()) {
