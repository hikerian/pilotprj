...
public class Type1DataRow {
	...
	public static Type1DataRow convert(UiElements uiElements) {
		String id = Type1DataRow.genId(uiElements);
		...
		return Type1DataRow.convert(id, selector, classNames, left, top, width, height);
	}
	
	public static Type1DataRow convert(String id, JSONObject target) {
	...
	private static Type1DataRow convert(String id, String selector, String[] classNms, double left, double top, double width, double height) {
		if(width == 0D || height == 0D) {
			// 크기가 0이면 skip!
			return null;
		}
		
		Type1DataRow dataRow = new Type1DataRow();
		dataRow.setId(id);
		
		dataRow.setGrpTitle(Type1DataRow.isContains(Type1DataRow.GROUPTITLE_CLASSES, selector));
		...
		dataRow.setDataTitleEl(Type1DataRow.isContains("data-title", classNms));
		
		dataRow.setButtonEl(Type1DataRow.isContains(Type1DataRow.BUTTON_CLASSES, classNms));
		dataRow.setInputEl(Type1DataRow.isContains(Type1DataRow.INPUT_CLASSES, classNms));
		dataRow.setOutputEl(Type1DataRow.isContains(Type1DataRow.OUTPUT_CLASSES, classNms));
	...
	public DataRow toDataRow() {
		...
		row.setData(new DoubleFeature(this.grpTitle ? 1D : 0D)
	...
	public String toString() {
		return "Type1DataRow [id=" + id + ", grpTitle=" + grpTitle + ", gridHeader=" + gridHeader + ", gridDetail=" + gridDetail
