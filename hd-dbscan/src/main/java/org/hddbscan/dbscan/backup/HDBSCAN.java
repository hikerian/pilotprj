...
public class HDBSCAN {
	...
	public DBSCANModel fit2(final DataSet inputValues) {
		// TODO 1depth 군집화 -> 군집화된 클러스터에서 2depth 군집화 -> 이렇게 되면 모든 요소가 군집을 이루게 되는 문제를
		// 고려해야 함.
		...
		final DBSCAN dbscan = new DBSCAN();
		List<DataRow> dataRowList = inputValues.getAllRows();

		List<DBSCANCluster> clusterList = new ArrayList<>();
		clusterList.add(new DBSCANCluster(dataRowList));
		...
		for (int i = 0; i < colCnt; i++) {
			...
			for (DBSCANCluster cluster : clusterList) {
				List<DBSCANCluster> newClusterList = dbscan.fit(cluster.getDataList(), i, eps, minPts);
		...
		final DBSCANModelBuilder modelBuilder = new DBSCANModelBuilder(colCnt);
		modelBuilder.setLabels(inputValues.getLabels()).setMetadata(this.metadata);

		DBSCANModel model = modelBuilder.build(clusterList);
